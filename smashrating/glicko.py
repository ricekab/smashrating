...
from typing import Any, Tuple, Collection, Iterable, Hashable, Dict
import math
from collections import defaultdict

DEFAULT_BASE_R = BASE_R = 1500  # Initial rating
DEFAULT_BASE_RD = BASE_RD = 350  # Maximum (and Initial) rating deviation
DEFAULT_MINIMUM_RD = 30.0

DEFAULT_C_FACTOR = C = 100.0  # Deviation factor when inactive
...
# C = 100.0
# (identical to 1-month period, 12 months reset)
...
class Rating(object):
    ...
    def update_deviation(
        ...
        max_rd: float = DEFAULT_BASE_RD,
        min_rd: float = DEFAULT_MINIMUM_RD,
    ):
        """
        The deviation is increased at the beginning of a rating period.

        :param c_factor: Determines the amount of deviation to increase. See
            glicko.pdf for details.

        :param deviation_factor: Inactivity factor. Value must be in range [0.0; 1.0],
            where 1.0 represents inactivity (no games played) and 0.0 represents
            enough activity to prevent any increase in deviation.
        """
        if not 0.0 <= deviation_factor <= 1.0:
            raise ValueError(
                f"Inactivity factor must be a value between 0.0 "
                f"and 1.0. (Given: {deviation_factor} ."
            )
        self.rd = min(
            math.sqrt(self.rd**2 + (c_factor * deviation_factor) ** 2), max_rd
        )
        self.rd = max(self.rd, min_rd)  # Ensure it is above the threshold.
...
def update_deviation(
    player_ratings: Dict[Hashable, Rating],
    c_factor: float = DEFAULT_C_FACTOR,
    deviation_factors: Dict[Hashable, float] = None,
        max_rd: float = DEFAULT_BASE_RD,
        min_rd: float = DEFAULT_MINIMUM_RD,
    ...
    :param c_factor: Determines the amount of deviation to increase. See glicko.pdf
        for details. Defaults to 100.0 .
    ...
    NOTE: This deviation factor is not part of the original glicko algorithm.
    glicko normally ALWAYS updates deviations.
    ...
    for player, rating in player_ratings.items():
        deviation_factor = deviation_factors.get(player, 1.0)
        rating.update_deviation(c_factor=c_factor, deviation_factor=deviation_factor,
                                max_rd=max_rd, min_rd=min_rd)
    return player_ratings


def update_ranking(
    sets: Iterable[Tuple[Hashable, Hashable]],
    player_ratings: Dict[Hashable, Rating] = None,
) -> Dict[Hashable, Rating]:
    """
    ...
    :return: The new rating period data.
    """
    ...
    # TODO: PROBLEM?? The Ratings are referenced, not copied?? To be checked.
    #   Can mutate data outside of function!
    # Post-period ratings, stays identical for players that haven't played.
    new_ratings = dict(old_ratings)
    # Note: Value-by-reference is OK because we will create new Rating objects later.
    #   Non-participating players will re-use the same reference.
    ...
    for p in players:
        ...
        # Note(Kevin): This step should not be necessary as the deviation update prior 
        #   to ranking updates enforces this.
...
def _g(rd):
    """g(RD) in https://www.glicko.net/glicko/glicko.pdf"""
